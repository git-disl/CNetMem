...
struct rdmabox_msg* get_free_msg()
{
   ...
   if (alf_queue_empty(cnetmem_page_pool.msg_freepool_list)){
   ...
   if (alf_mc_dequeue(cnetmem_page_pool.msg_freepool_list, (void **)&tmp, 1) == 0){
	return NULL;
     }
    //printk("mempool[%s]:Get free item free_item_count=%u \n",__func__,
    //         alf_queue_count(cnetmem_page_pool.msg_freepool_list));
out:
    return tmp;
}
...
{  
    if (alf_mp_enqueue(cnetmem_page_pool.msg_freepool_list, (void **)&tmp, 1) != 1){
...
/*
   if (alf_queue_empty(cnetmem_page_pool.freepool_list)){
...
/*
   ...
   if (alf_mc_dequeue(cnetmem_page_pool.freepool_list, (void **)&tmp, 1) == 0){
	return NULL;
     }
    //printk("mempool[%s]:Get free item free_item_count=%u \n",__func__,
    //         alf_queue_count(cnetmem_page_pool.freepool_list));
out:
*/
    return tmp;
}
...
{  
/*
    if (alf_mp_enqueue(cnetmem_page_pool.freepool_list, (void **)&tmp, 1) != 1){
	printk("mempool[%s]: Fail to put free_item \n", __func__);
	//vfree(tmp);
*/
	kfree(tmp);
...
{
    if(unlikely(cnetmem_page_pool.init_done == 0)){
	return 1;
    }
...
{
    if (alf_mp_enqueue(cnetmem_page_pool.read_req_list, (void **)&tmp, 1) != 1){
	printk("mempool[%s]: Fail to put req item \n",__func__);
	return 1;
    }
    //printk("mempool[%s]:put item count=%u \n", __func__,alf_queue_count(cnetmem_page_pool.req_list));
    return 0;
}

int get_read_req_item(void **tmp)
{
    if (alf_queue_empty(cnetmem_page_pool.read_req_list)){
	return 1;
    }

    if (alf_mc_dequeue(cnetmem_page_pool.read_req_list, (void **)tmp, 1) == 0){
	return 1;
    }
    //printk("mempool[%s]:get item count=%u \n", __func__,alf_queue_count(cnetmem_page_pool.req_list));
    return 0;
...
{
    if (alf_mp_enqueue(cnetmem_page_pool.req_list, (void **)&tmp, 1) != 1){
	printk("mempool[%s]: Fail to put req item \n",__func__);
	return 1;
    }
    //printk("mempool[%s]:put item count=%u \n", __func__,alf_queue_count(cnetmem_page_pool.req_list));
...
{
    if (alf_queue_empty(cnetmem_page_pool.req_list)){
	return 1;
    }

    if (alf_mc_dequeue(cnetmem_page_pool.req_list, (void **)tmp, 1) == 0){
	return 1;
    }
    //printk("mempool[%s]:get item count=%u \n", __func__,alf_queue_count(cnetmem_page_pool.req_list));
    return 0;
...
{
    if (alf_mp_enqueue(cnetmem_page_pool.sending_list, (void **)&tmp, 1) != 1){
	printk("mempool[%s]: Fail to put sending index \n",__func__);
	return 1;
    }
    return 0;
...
{
    if (alf_queue_empty(cnetmem_page_pool.sending_list)){
	return 1;
    }

    if (alf_mc_dequeue(cnetmem_page_pool.sending_list, (void **)tmp, 1) == 0){
	return 1;
    }
    return 0;
}
...
int put_reclaimable_index(void *tmp)
{
    ...
    //spin_lock_irqsave(&cnetmem_page_pool.rlock, flags);
    if (alf_mp_enqueue(cnetmem_page_pool.reclaim_list, (void **)&tmp, 1) != 1){
	printk("mempool[%s]: Fail to put reclaimable index \n",__func__);
        //spin_unlock_irqrestore(&cnetmem_page_pool.rlock, flags);
	return 1;
    }
    //spin_unlock_irqrestore(&cnetmem_page_pool.rlock, flags);

   // printk("mempool[%s]:Put Reclaimable item start_index=%zu len=%d reclaimable_count=%u \n",__func__,
   //          tmp->start_index,tmp->len,alf_queue_count(cnetmem_page_pool.reclaim_list));
...
int get_reclaimable_index(void **tmp)
{
    ...
    //spin_lock_irqsave(&cnetmem_page_pool.rlock, flags);
    if (alf_queue_empty(cnetmem_page_pool.reclaim_list)){
        //spin_unlock_irqrestore(&cnetmem_page_pool.rlock, flags);
	return 1;
    }

    if (alf_mc_dequeue(cnetmem_page_pool.reclaim_list, (void **)tmp, 1) == 0){
        //spin_unlock_irqrestore(&cnetmem_page_pool.rlock, flags);
	return 1;
    }
    //spin_unlock_irqrestore(&cnetmem_page_pool.rlock, flags);
...
static void cnetmem_add_element(cnetmem_mempool_t *pool, void *element)
{
	unsigned long flags;

        spin_lock_irqsave(&pool->lock, flags);
        if(unlikely(pool->curr_nr >= pool->cap_nr)){
            //panic("cnetmem_add_element pool->curr_nr >= pool->cap_nr");
...
static void *cnetmem_remove_element(cnetmem_mempool_t *pool)
{
	unsigned long flags;
	void * element;
        ...
        if (pool->curr_nr > 0) {
           if(unlikely(pool->curr_nr <= 0)){
                panic("cnetmem_add_element pool->curr_nr <= 0");
           }
...
// real delete
static void cnetmem_free_pool(cnetmem_mempool_t *pool)
{
	while (pool->curr_nr) {
		void *element = cnetmem_remove_element(pool);
		__free_pages(element,0);
	}
	//kfree(pool->elements);
	vfree(pool->elements);
...
void * cnetmem_alloc()
{
    return cnetmem_mempool_alloc(&cnetmem_page_pool);
}

void * cnetmem_mempool_alloc(cnetmem_mempool_t *pool)
{
	void *element;
	unsigned long flags;
        int err;
	...
	element = cnetmem_remove_element(pool);
	if(!element){
repeat:
		element = cnetmem_mempool_reclaim(pool);
		if(!element){
...
void cnetmem_free(void *element)
{
    cnetmem_mempool_free(element, &cnetmem_page_pool);
}

void cnetmem_mempool_free(void *element, cnetmem_mempool_t *pool)
{
	unsigned long flags;
	...
	cnetmem_add_element(pool, element);
}

void * cnetmem_mempool_reclaim(cnetmem_mempool_t *pool)
{
...
//cnetmem_mempool_t *cnetmem_mempool_create(cnetmem_mempool_t *pool, int cap_nr)
int cnetmem_mempool_create(cnetmem_mempool_t *pool, long cap_nr)
{
        unsigned long flags;
...
static int mempool_thread_fn(void *data)
{
    ...
    while(true)
    {
	   ...
	   if(pool->cap_nr <= RESIZING_UNIT_IN_PAGES){
           ...
           err = cnetmem_mempool_shrink(pool, pool->new_cap_nr);
           if(err){
...
*/
	 	    ...
	 	    pool->new_cap_nr = pool->cap_nr + RESIZING_UNIT_IN_PAGES;
                    ...
                    err = cnetmem_mempool_expand(pool, pool->new_cap_nr);
                    if(err){
...
int cnetmem_mempool_shrink(cnetmem_mempool_t *pool, long new_cap_nr)
{
	void *element;
	int i;
	unsigned long flags;
...
// TODO: resize should be fixed later. It needs to be dynamic resizing
//       without interference of pre-allocated mem.
int cnetmem_mempool_expand(cnetmem_mempool_t *pool, long new_cap_nr)
{
	void *element;
	void **new_elements;
	void **tmp_elements;
...
// TODO : dest alf_queue
void cnetmem_mempool_destroy(cnetmem_mempool_t *pool)
{
	// TODO : why need ?
	...
	cnetmem_free_pool(pool);
	// copy cnetmem_free_pool below instead of calling
	while (pool->curr_nr) {
		void *element = cnetmem_remove_element(pool);
		__free_pages(element,0);
	}
...
static void set_mempool_size(size_t request_min, size_t request_max)
{
  ...
  // set min_pool_pages
  if (!request_min) {
      cnetmem_page_pool.min_pool_pages = default_mempool_min * (freemem_pages / 100);
      printk("mempool: Using default for Min Mempool Size (%d%% of RAM) \n", default_mempool_min);
  }else{
    cnetmem_page_pool.min_pool_pages = request_min;
  }
  printk("mempool: FreeMem Size: %zu GB \nmempool: Min Mempool Pages: %zu \nmempool: Min Mempool size: %zu GB \n",
         (freemem_pages * (size_t)info.mem_unit) >> 30, cnetmem_page_pool.min_pool_pages, (cnetmem_page_pool.min_pool_pages * (size_t)info.mem_unit) >> 30
  );

  // set max_pool_pages
  if (!request_max) {
      cnetmem_page_pool.max_pool_pages = default_mempool_max * (freemem_pages / 100);
      printk("mempool: Using default for Max Mempool Size (%d%% of RAM) \n", default_mempool_max);
  }else{
    cnetmem_page_pool.max_pool_pages = request_max;
  }
  printk("mempool: FreeMem Size: %zu GB \nmempool: Max Mempool Pages: %zu \nmempool: Max Mempool size: %zu GB \n",
         (freemem_pages * (size_t)info.mem_unit) >> 30, cnetmem_page_pool.max_pool_pages, (cnetmem_page_pool.max_pool_pages * (size_t)info.mem_unit) >> 30
  );
}

int is_mempool_init_done()
{
    return cnetmem_page_pool.init_done;
}
...
int mempool_init()
{
    ...
    cnetmem_page_pool.init_done = 0;

    spin_lock_init(&cnetmem_page_pool.lock);
    spin_lock_init(&cnetmem_page_pool.slock);
    spin_lock_init(&cnetmem_page_pool.rlock);
    spin_lock_init(&cnetmem_page_pool.flock);
    ...
    printk("mempool[%s]: mempool init start. mempool min pages:%zu mempool max pages=%zu submit_queues:%d \n",__func__,cnetmem_page_pool.min_pool_pages, cnetmem_page_pool.max_pool_pages,submit_queues);

    err = cnetmem_mempool_create(&cnetmem_page_pool, cnetmem_page_pool.min_pool_pages);
    if (err){
    ...
    cnetmem_page_pool.sending_list = alf_queue_alloc(cnetmem_page_pool.min_pool_pages, GFP_KERNEL);
    if (IS_ERR_OR_NULL(cnetmem_page_pool.sending_list)){
       printk("mempool[%s]: Fail to alloc sending_list \n",__func__);
        
       goto fail;
    }
    cnetmem_page_pool.reclaim_list = alf_queue_alloc(cnetmem_page_pool.min_pool_pages, GFP_KERNEL);
    if (IS_ERR_OR_NULL(cnetmem_page_pool.reclaim_list)){
       printk("mempool[%s]: Fail to alloc reclaim_list \n", __func__);
        
       goto fail;
    }  
/*
    cnetmem_page_pool.freepool_list = alf_queue_alloc(submit_queues*RDMABOX_QUEUE_DEPTH, GFP_KERNEL);
    if (IS_ERR_OR_NULL(cnetmem_page_pool.freepool_list)){
       printk("mempool[%s]: Fail to alloc freepool_list \n",__func__);
        
       goto fail;
    }  

    cnetmem_page_pool.msg_freepool_list = alf_queue_alloc(submit_queues*RDMABOX_QUEUE_DEPTH, GFP_KERNEL);
    if (IS_ERR_OR_NULL(cnetmem_page_pool.msg_freepool_list)){
       printk("mempool[%s]: Fail to alloc freepool_list \n",__func__);
...
*/
    // write req list
    cnetmem_page_pool.req_list = alf_queue_alloc(submit_queues*RDMABOX_QUEUE_DEPTH, GFP_KERNEL);
    if (IS_ERR_OR_NULL(cnetmem_page_pool.req_list)){
       printk("mempool[%s]: Fail to alloc req_list \n",__func__);
        
       goto fail;
    }  
    // read req list
    cnetmem_page_pool.read_req_list = alf_queue_alloc(submit_queues*RDMABOX_QUEUE_DEPTH, GFP_KERNEL);
    if (IS_ERR_OR_NULL(cnetmem_page_pool.req_list)){
       printk("mempool[%s]: Fail to alloc req_list \n",__func__);
    ...
    cnetmem_page_pool.mempool_thread = kthread_create(mempool_thread_fn, &cnetmem_page_pool, thread_name);
    wake_up_process(cnetmem_page_pool.mempool_thread);

    cnetmem_page_pool.init_done = 1;
